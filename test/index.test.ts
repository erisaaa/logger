...
describe('@erisa/logger', () => {
    ...
    for (const level of Object.keys(loggerExt.levels))
      specify(level, () => {
        ...
        const result = stdout.inspectSync(() =>
          loggerExt.dispatch(level, testString)
        );

        expect(result.length).to.equal(1);
        expect(result[0].trim()).to.equal(
          `${lvl.tagText} ${lvl.textFunc(testString)}`
        );
      });
  });
  ...
  it('should print regular text when a non-existant level is passed', () => {
    const result = stdout.inspectSync(() =>
      loggerExt.dispatch('foo', testString)
    );
  ...
  describe('defaultListeners', () => {
    ...
    describe('logging events', () => {
      ...
      for (const [event, [before, args, expected, level]] of Object.entries(
        logEvents
      ))
        specify(event, () => {
